...
void AFirstPersonCharacter::BeginPlay()
{
	...
	auto World = GetWorld();
	if (ensure(World != NULL && GunClass != NULL))
		FP_Gun = World->SpawnActor<AGun>(GunClass, Mesh1P->GetSocketLocation(FName("GripPoint")), Mesh1P->GetSocketRotation("GripPoint"));
	//Attach gun mesh component to Skeleton, doing it here because the skeleton is not yet created in the constructor
	if (ensure(FP_Gun != NULL))
	{
...
void AFirstPersonCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	FP_InputComponent = PlayerInputComponent;
